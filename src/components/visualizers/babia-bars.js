        ...
        self.newData.forEach(bar => {
            if (!bar._not){
                if (document.getElementById(bar[self.data.index])){
                    let pos_x = document.getElementById(bar[self.data.index]).getAttribute('position').x
                    document.getElementById(bar[self.data.index]).setAttribute('height', bar[self.data.height])
                    document.getElementById(bar[self.data.index]).setAttribute('position', {x: pos_x, y: bar[self.data.height]/2, z:0})
                    console.log(document.getElementById(bar[self.data.index]))
                } else {
...
let generateBarChart = (self, data, dataRetrieved, element, animation, chart, list, widthBars) => {
    if (dataRetrieved) {
        ...
        for (let bar of dataToPrint) {
            ...
            let barEntity = generateBar(self, bar[self.data.height], widthBars, colorid, stepX, palette, animation, scale, list);
            barEntity.id = bar[self.data.index]
            console.log(barEntity);
